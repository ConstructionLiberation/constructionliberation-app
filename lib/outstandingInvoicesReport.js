import { fromEmail, companyName, logoUrl as tenantLogoUrl } from './tenantSettings'
// Server side can read the tenant directly - no feed needed. This file
// already imports tenantSettings, so it is always inside a tenant context.
import { money as tenantMoney } from './locale'
import { getPortalUsers } from './db'
import { getClient } from './db'
import { normRole } from './roles'
import { buildOutstandingInvoicesPDF } from './outstandingInvoicesPdf'
import { CHASE_STAGE_LABELS } from './chaseEmailTemplates'


const parseDMY = (s) => {
  if (!s) return null
  if (/^\d{4}-\d{2}-\d{2}/.test(s)) return new Date(s)
  const m = /^(\d{1,2})\/(\d{1,2})\/(\d{4})/.exec(s)
  if (m) return new Date(+m[3], +m[2] - 1, +m[1])
  const d = new Date(s); return isNaN(d) ? null : d
}

const RECIPIENTS_KEY = 'invoice:weekly-recipients'   // extra external emails [str]
const SCHEDULE_KEY = 'invoice:weekly-schedule'       // { dayOfWeek:0-6, hour:0-23, lastSentDate:'YYYY-MM-DD' }
const META_KEY = 'invoice:meta'

// Weekly report schedule. Defaults to Thursday (4) at 08:00 to match old behaviour.
export async function getWeeklySchedule() {
  const redis = await getClient()
  const s = redis ? (await redis.get(SCHEDULE_KEY)) : null
  return { dayOfWeek: s?.dayOfWeek ?? 4, hour: s?.hour ?? 8, lastSentDate: s?.lastSentDate || '' }
}
export async function setWeeklySchedule({ dayOfWeek, hour }) {
  const redis = await getClient()
  const cur = await getWeeklySchedule()
  const next = {
    dayOfWeek: Math.max(0, Math.min(6, parseInt(dayOfWeek))),
    hour: Math.max(0, Math.min(23, parseInt(hour))),
    lastSentDate: cur.lastSentDate || '',
  }
  if (redis) await redis.set(SCHEDULE_KEY, next)
  return next
}
async function markScheduleSent(dateStr) {
  const redis = await getClient()
  const cur = await getWeeklySchedule()
  if (redis) await redis.set(SCHEDULE_KEY, { ...cur, lastSentDate: dateStr })
}

// Called by the hourly cron. Sends the report iff the configured day + hour match
// "now" (UK time) and it hasn't already been sent today. force bypasses the gate.
export async function maybeSendScheduledReport({ baseUrl, force, dryRun } = {}) {
  const sched = await getWeeklySchedule()
  // Reliable UK local day/hour via Intl parts (robust across BST/GMT; the cron ticks
  // hourly in UTC, so we must evaluate the gate in UK time).
  const parts = new Intl.DateTimeFormat('en-GB', {
    timeZone: 'Europe/London', weekday: 'short', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', hour12: false,
  }).formatToParts(new Date())
  const get = (t) => parts.find(p => p.type === t)?.value || ''
  const wdMap = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 }
  const DAYS = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']
  const ukDay = wdMap[get('weekday')]
  let ukHour = parseInt(get('hour'), 10); if (ukHour === 24) ukHour = 0
  const dateStr = `${get('year')}-${get('month')}-${get('day')}`

  const diag = {
    nowUkDay: ukDay, nowUkDayName: DAYS[ukDay], nowUkHour: ukHour, nowUkDate: dateStr,
    wantDay: sched.dayOfWeek, wantDayName: DAYS[sched.dayOfWeek], wantHour: sched.hour,
    lastSentDate: sched.lastSentDate || '(never)',
  }

  if (!force) {
    if (ukDay !== sched.dayOfWeek) { const r = { skipped: `Not the scheduled day - today is ${DAYS[ukDay]} (UK), schedule is ${DAYS[sched.dayOfWeek]}`, diag }; return r }
    if (ukHour < sched.hour) { const r = { skipped: `Too early - it's ${ukHour}:00 UK, scheduled for ${sched.hour}:00`, diag }; return r }
    if (sched.lastSentDate === dateStr) { const r = { skipped: `Already sent today (${dateStr})`, diag }; return r }
  }
  if (dryRun) return { wouldSend: true, diag }
  const result = await sendWeeklyOverdueReport({ baseUrl })
  if (result && result.ok) await markScheduleSent(dateStr)
  return { ...result, diag }
}


// chases are a map of stageKey -> { at, to[], subject, manual }. The PDF wants
// a chronological list of { at, type, to } with `to` as a string.
function chaseList(chases) {
  return Object.entries(chases || {})
    .map(([stageKey, c]) => ({
      at: (c && c.at) || 0,
      type: CHASE_STAGE_LABELS[stageKey] || stageKey,
      to: (Array.isArray(c && c.to) ? c.to : [c && c.to]).filter(Boolean).join(', '),
      subject: (c && c.subject) || '',
    }))
    .sort((a, b) => (a.at || 0) - (b.at || 0))
}

// Assemble the outstanding-invoice rows from the cached dashboard projects + meta.
export async function gatherOutstandingInvoices({ baseUrl } = {}) {
  const redis = await getClient()
  const meta = redis ? ((await redis.get(META_KEY)) || {}) : {}

  // Read the cached dashboard (avoid recomputation / Xero calls).
  let projects = []
  try {
    if (redis) {
      const cached = await redis.get('dashboard:cache')
      if (cached) projects = cached
    }
    if (!projects.length && baseUrl) {
      const d = await fetch(`${baseUrl}/api/dashboard`).then(r => r.json())
      projects = d.projects || []
    }
  } catch {}

  const today = new Date(); today.setHours(0, 0, 0, 0)
  const rows = []
  for (const p of projects) {
    for (const inv of (p._invoiceLines || [])) {
      const due = inv.amountDue != null ? inv.amountDue : ((inv.total || 0) - (inv.amountPaid || 0))
      if (!(due > 0.005)) continue
      const dd = parseDMY(inv.dueDate)
      const overdueBy = dd ? Math.floor((today - dd) / 86400000) : 0
      const m = meta[inv.invoiceNumber] || {}
      rows.push({
        invoiceNumber: inv.invoiceNumber || '',
        reference: inv.reference || p.name || '',
        customer: inv.contact || p.customer || '',
        date: inv.date || '',
        dueDate: inv.dueDate || '',
        overdueBy: overdueBy > 0 ? overdueBy : null,
        expectedDate: m.expectedDate || '',
        paid: inv.amountPaid || 0,
        due,
        highRisk: !!p.highRisk,
        qsName: p.qsName || '',
        comments: m.comments || [],
        // `m.emails` DOES NOT EXIST and never has. Chases are stored under
        // m.chases, keyed by stage, by the record-chase action in
        // pages/api/outstanding-invoices.js. This line read a field nothing
        // writes, so every invoice in the weekly PDF said "No chase emails
        // sent yet" however many had gone out - while the email body beside
        // it promised "comments and emails to date for each invoice".
        //
        // Reshaped here into what the PDF renders: at, type, to.
        emails: chaseList(m.chases),
      })
    }
  }
  rows.sort((a, b) => (b.overdueBy || -1) - (a.overdueBy || -1))
  return rows
}

export async function getWeeklyRecipients() {
  const redis = await getClient()
  if (!redis) return []
  return (await redis.get(RECIPIENTS_KEY)) || []
}
export async function setWeeklyRecipients(list) {
  const redis = await getClient()
  if (redis) await redis.set(RECIPIENTS_KEY, list)
}

// Internal recipients: portal users with post-contract / management / admin access.
async function internalRecipients() {
  const users = await getPortalUsers()
  const allowed = ['post-contract', 'management', 'admin']
  return users
    .filter(u => u.active !== false && u.email && allowed.includes(normRole(u.role)))
    .map(u => u.email)
}

// Build + send the weekly report. Returns a summary object.
export async function sendWeeklyOverdueReport({ baseUrl, extraRecipients, dryRun } = {}) {
  const RESEND_KEY = process.env.RESEND_API_KEY
  const FROM = fromEmail('forms')

  const invoices = await gatherOutstandingInvoices({ baseUrl })
  const external = extraRecipients || await getWeeklyRecipients()
  // The managed recipient list (portal users you add + manual emails) is the
  // definitive list. If it's empty, fall back to internal auto-recipients so the
  // report is never sent to nobody.
  let to = [...new Set((external || []).filter(Boolean))]
  if (!to.length) {
    const internal = await internalRecipients()
    to = [...new Set(internal.filter(Boolean))]
  }

  // Clear, self-diagnosing failures rather than a silent non-send.
  if (!RESEND_KEY) return { ok: false, reason: 'RESEND_API_KEY is not set', sentTo: to, from: FROM }
  if (!to.length) return { ok: false, reason: 'No recipients: add emails to the weekly recipient list, or give a post-contract/management/admin portal user an email address', sentTo: [], from: FROM }

  // The tenant's logo, not Rock's. No `req` here - this runs from a cron - so
  // the tenant's own value is used when it is absolute, and made absolute
  // against baseUrl when it is a path.
  const rawLogo = String(tenantLogoUrl() || '').trim()
  const logoUrl = !rawLogo ? null
    : (/^https?:\/\//i.test(rawLogo) ? rawLogo : (baseUrl ? `${baseUrl}${rawLogo.startsWith('/') ? '' : '/'}${rawLogo}` : null))
  const pdfBytes = await buildOutstandingInvoicesPDF({ invoices, includeComments: true, logoUrl })
  const b64 = Buffer.from(pdfBytes).toString('base64')

  const overdue = invoices.filter(i => i.overdueBy)
  // Was hardcoded en-GB/GBP, so the weekly chase report arrived in sterling
  // whoever it was for. Two decimal places, which is what the old one did -
  // Intl's currency style defaults to the currency's own digits, so the
  // absence of a fractionDigits option there did NOT mean whole pounds.
  const money = (n) => tenantMoney(n || 0)
  const dateLabel = new Date().toLocaleDateString('en-GB')
  const fmtDue = (s) => {
    if (!s) return '—'
    let d = null
    if (/^\d{4}-\d{2}-\d{2}/.test(s)) d = new Date(s)
    else { const m = /^(\d{1,2})\/(\d{1,2})\/(\d{4})/.exec(s); if (m) d = new Date(+m[3], +m[2] - 1, +m[1]) }
    return d && !isNaN(d) ? d.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : String(s)
  }

  if (dryRun || !RESEND_KEY) return { ok: !!dryRun, wouldSendTo: to, invoiceCount: invoices.length, note: RESEND_KEY ? 'dry run' : 'email not configured', pdfBytes }

  const rowsHtml = invoices.map(i => `
      <tr>
        <td style="padding:6px 8px;border-bottom:1px solid #eee;font-weight:600">${i.invoiceNumber || '—'}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee">${i.reference || '—'}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee">${i.customer || '—'}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee;color:${i.overdueBy ? '#dc2626' : '#333'}">${fmtDue(i.dueDate)}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee;color:#dc2626;font-weight:600">${i.overdueBy ? i.overdueBy + ' days' : ''}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right">${money(i.due)}</td>
      </tr>`).join('')

  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;max-width:720px;margin:0 auto;color:#1a1a19">
      <h2 style="color:#1a1a19">${companyName()} Outstanding Invoices — Weekly Report (${dateLabel})</h2>
      <p style="font-size:15px">${invoices.length} outstanding invoice${invoices.length === 1 ? '' : 's'}${overdue.length ? `, of which <span style="color:#dc2626">${overdue.length} overdue</span>` : ''}.</p>
      <table style="border-collapse:collapse;width:100%;font-size:13px;margin-top:8px">
        <thead>
          <tr style="background:#f5f5f5">
            <th style="padding:6px 8px;text-align:left;border-bottom:2px solid #ddd">Inv Number</th>
            <th style="padding:6px 8px;text-align:left;border-bottom:2px solid #ddd">Ref</th>
            <th style="padding:6px 8px;text-align:left;border-bottom:2px solid #ddd">To</th>
            <th style="padding:6px 8px;text-align:left;border-bottom:2px solid #ddd">Due Date</th>
            <th style="padding:6px 8px;text-align:left;border-bottom:2px solid #ddd">Overdue</th>
            <th style="padding:6px 8px;text-align:right;border-bottom:2px solid #ddd">Due</th>
          </tr>
        </thead>
        <tbody>${rowsHtml}</tbody>
      </table>
      <p style="font-size:13px;color:#777;margin-top:14px">The full list, with comments and emails to date for each invoice, is attached as a PDF.</p>
    </div>`

  const resp = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${RESEND_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      from: FROM, to, subject: `${companyName()} Outstanding Invoices - Weekly Report (${dateLabel})`,
      html, attachments: [{ filename: `Outstanding-Invoices-${new Date().toISOString().slice(0, 10)}.pdf`, content: b64 }],
    }),
  })
  let errorText = ''
  if (!resp.ok) { try { errorText = JSON.stringify(await resp.json()) } catch { try { errorText = await resp.text() } catch {} } }
  return { ok: resp.ok, sentTo: to, invoiceCount: invoices.length, status: resp.status, from: FROM, error: errorText || undefined }
}
