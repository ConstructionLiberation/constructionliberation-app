...
function projectCacheRow(p) {
  ...
  return {
    ...
    // THE CONTACT THE VARIATION AND APPLICATION EMAILS ADDRESS.
    //
    // customerContacts was seeded as an EMPTY ARRAY.
    // components/VariationBuilder.js reads project.customerContacts for the
    // recipient list AND for the greeting name, so with none there was nobody
    // to send to and the greeting fell back to the company - "Hi Harbourline"
    // instead of "Hi Jenna".
    //
    // Two per project: the person who signs things off and somebody in
    // accounts, so the CC list has a choice in it and selecting one is
    // visibly different from selecting both.
    customerContacts: [
      { name: p.contact, email: personEmail(p.contact), role: 'Quantity Surveyor' },
      { name: pick(rng(p.i + 31), CONTACTS.filter(c => c !== p.contact)), email: '', role: 'Accounts' },
    ].map((c) => ({ ...c, email: c.email || personEmail(c.name) })),
    qsEmail: '',
    customerEmail: personEmail(p.contact),
...
// work priced".
// The day a deal was won or lost: somewhere between halfway through its life
// and almost now, so decisions land across the months rather than all at once.
function decidedOn(r, addedTs) {
  const t = addedTs + Math.round((Date.now() - addedTs) * between(r, 0.25, 0.98))
...
// The order a deal moves through - used to walk its history. Unweighted.
const DEAL_STAGES = [
...
// The same stages, weighted for PICKING one. Early stages hold the most deals;
// MC Secured and Negotiating carry enough for the secured-value metrics to
// have something to measure every month.
const STAGE_WEIGHTS = [
  'stage_project_in', 'stage_project_in',
  'stage_1st_contact', 'stage_1st_contact',
  'stage_calls_x3', 'stage_in_abeyance', 'stage_tbf',
  'stage_mc_unsec_np', 'stage_info_pending', 'stage_received',
  'stage_1', 'stage_2', 'stage_review',
  'stage_mc_unsecured', 'stage_mc_unsecured',
  'stage_variations',
  'stage_mc_secured', 'stage_mc_secured', 'stage_mc_secured', 'stage_mc_secured',
  'stage_negotiating', 'stage_negotiating', 'stage_negotiating',
]
...
// A DISTINCT ADDRESS PER PERSON THAT STILL REACHES ONE INBOX.
//
// Everyone shared DEMO_EMAIL, so anything that identifies a person BY EMAIL
// treated all nine as the same person. Ticking one name in the variation
// CC list ticked every name, because the checkbox is
// `cc.includes(c.email)` - correct code, nine identical identities. It is
// also what sent four copies of the same daily email in pkg973, where I
// deduped the sending and never followed the cause back here.
//
// Plus-addressing gives a genuinely distinct address that still lands in the
// same mailbox, so the demo keeps its one safe destination and the app sees
// nine different people.
//
//   james+callum.reid@constructionliberation.com
const personEmail = (name) => {
  const slug = String(name || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '.').replace(/^\.|\.$/g, '')
  if (!slug) return DEMO_EMAIL
...
function buildDeals(r, count) {
  ...
  for (let i = 0; i < count; i++) {
    // A REAL PIPELINE IS NOT FLAT. Picking uniformly from fourteen stages put
    // only 4/14 of deals in a stage that can be WON, which left "total value
    // of work secured" with too few wins to plot a rolling six-month figure.
    // Weighted so the late stages carry the volume a converting pipeline has,
    // while the early stages still hold the most deals overall.
    const stage = pick(r, STAGE_WEIGHTS)
    ...
    // TWENTY MONTHS, NOT THIRTEEN.
    // The scorecard charts a rolling 12 months and each point is itself a
    // rolling 6-month window, so it needs deals decided up to 18 months back
    // to draw the earliest points. With a 13-month spread the left-hand half
    // of every chart was empty.
    const addedTs = monthsBack(intBetween(r, 0, 20)).getTime()
    // WON OR LOST DEPENDS ON AGE AND STAGE, NOT ON A FLAT 12%.
    //
    // The old rule decided only the MC Secured deals plus 12% of everything
    // else - 38 out of 190, about thirteen per estimator spread over fifteen
    // months. Strike rate and value secured are computed over a ROLLING SIX
    // MONTHS, so most windows contained nothing at all and the chart simply
    // had no point to plot. That is why the left half of every one of those
    // charts was empty however far back the deals were dated.
    //
    // It was also not believable: a twenty-month-old deal still sitting open
    // is not something a real pipeline contains. Old deals get decided, late
    // stages mostly convert, early ones quietly die.
    const ageMonths = (Date.now() - addedTs) / (1000 * 60 * 60 * 24 * 30.4)
    const LATE = ['stage_mc_unsecured', 'stage_variations', 'stage_mc_secured', 'stage_negotiating']
    const isLate = LATE.includes(stage)
    const roll = r()
    const status = isLate
      // 45/43/12. Tuned so the OVERALL strike rate lands near the 25% target
      // rather than comfortably above it - a scorecard that is green on every
      // card in every month demonstrates nothing about the scorecard.
      ? (ageMonths > 3 ? (roll < 0.45 ? 'won' : roll < 0.88 ? 'lost' : 'open') : 'open')
      : (ageMonths > 6 ? (roll < 0.5 ? 'lost' : 'open') : (roll < 0.08 ? 'lost' : 'open'))
    deals.push({
      id: `d${1000 + i}`,
      ...
      fields: {
        ...
        // A realistic mix. Existing customers are the minority of a pipeline
        // by count and the majority of it by comfort, which is exactly why
        // "value priced to existing customers" is a metric worth watching.
        customer_type: pick(r, [
          'Existing Customer', 'Existing Customer',
          'New Customer', 'New Customer', 'New Customer',
          'Prospect', 'Prospect', 'Prospect', 'Prospect',
        ]),
        // WHEN IT WAS DECIDED. Four blank metrics, one missing pair of fields.
        //
        // lib/crmDashboardAdapter.js builds:
        //     wonTime:   f.won_time
        //     closeTime: f.won_time || f.lost_time
        // and the scorecard hangs four things off them:
        //     monthWon        -> total value of work secured      (was $0)
        //     rolling6        -> strike rate, overall and MC       (was blank)
        //     avgSecuredWindow-> average secured project value     (was blank)
        //     dealsOver200k   -> deals secured over 200k           (was 0)
        //
        // The seeder set status to won or lost and never said WHEN, so every
        // one of those filters matched nothing. status alone is not a decision
        // - the date is.
        //
        // SPREAD, not clustered. A fixed fraction of the elapsed time closes
        // every old deal recently, which fills the latest month and leaves the
        // earlier ones empty - a trend line with one spike teaches nobody
        // anything. A random point between halfway and nearly-now gives a
        // decision in most months.
        ...(status === 'won' ? { won_time: decidedOn(r, addedTs) } : {}),
        ...(status === 'lost' ? { lost_time: decidedOn(r, addedTs) } : {}),
        lost_reason: status === 'lost' ? pick(r, LOST_REASONS) : null,
        organization: org,
        contact_person: contact,
        contact_email: personEmail(contact),
        ...
        // LEAD SOURCE, FROM THE REAL OPTION LIST.
        //
        // This was Referral / Tender list / Repeat client / Website / Cold
        // approach - plausible words, none of them an option in
        // lib/crmFieldSchema.js. So the dropdown on a deal offered one set and
        // the data held another, and two scorecard metrics that filter on the
        // source by name - "Glenigan scored 5 or more" and "Actively Chased
        // scored 5 or more" - could never match anything. Same mistake as the
        // lost reasons: invented values instead of the schema's.
        //
        // Weighted so Glenigan and Actively Chased both carry enough volume
        // for their metrics to move.
        lead_source: pick(r, [
          'Glenigan', 'Glenigan', 'Glenigan', 'Glenigan',
          'Actively Chased', 'Actively Chased', 'Actively Chased',
          'Website New Build', 'Website New Build',
          'Website Refurb',
          'Other',
        ]),
        // PROJECT SCORE. The same two metrics need a score of 5 or more, read
        // as parseInt(d.label) off f.project_score. Nothing set it, so every
        // score was NaN and both metrics read 0 however the lead source was
        // labelled. Skewed high - a project nobody rates above 4 is not one
        // anybody chases.
        project_score: String(intBetween(r, 3, 10)),
...
export function generateDemo(opts = {}) {
  ...
  const opsUsers = OPERATIVES.map((nm, i) => {
    ...
    return {
      ...
      opId: `op${100 + i}`,
      name: nm,
      email: personEmail(nm),
      phone: '02' + intBetween(r, 1000000, 9999999),
      // A PIN. Without one, pages/api/ops-users.js compares
      // String(user.pin) === String(body.pin) - and String(undefined) matches
      // nothing, so NOBODY could log into the Site App on demo. The only way
      // in was Forgot PIN.
      //
      // The same four digits for everyone on purpose: this is a demo tenant
      // whose whole point is being handed to someone, and remembering fifteen
      // PINs helps nobody. A real tenant sets its own on invite.
      pin: '1234',
      ...
      // 'all', NOT [].
      //
      // lib/cmSiteApp.js: `if (pa == null || pa === 'all') return projects`.
      // An empty ARRAY is neither, so it builds an empty Set and matches
      // nothing - every seeded operative had access to NO projects, which is
      // why the Site App's Schedule of Works was empty. [] reads like "no
      // restrictions" and means the exact opposite.
      projectAccess: 'all',
      training,
    }
  })
