...
function buildProject(r, i, opts) {
  ...
  // MARGIN IS MEASURED AGAINST WHAT WAS INVOICED, NOT WHAT WAS CERTIFIED.
  //
  // The page computes (invoiced - costs) / invoiced. Costs were being derived
  // from certifiedGross, which lags invoiced by a random 0-12%, so the
  // reported margin swung between -50% and +71% on a dataset meant to average
  // 25%. Derive from the same figure the page divides by and the number comes
  // out where it is supposed to.
  const totalCosts = Math.round(appliedForLatest * (1 - margin))
  // WIP IS COSTS AFTER THE VALUATION DATE, GROSSED UP AT THE MARGIN.
  //
  // The first version dated every cost line randomly across the project's
  // life, so almost nothing landed after the current month's valuation date
  // and the WIP page said "no WIP to show". A live project with no
  // post-valuation cost is not realistic anyway - work continues after the
  // valuation is cut.
  //
  // So live projects get a handful of lines dated deliberately between the
  // valuation date and today.
  const costLines = []
  ...
  // Post-valuation costs: dated after the 28th of last month, which is the
  // valuation date the WIP page will be using.
  // THE WIP WINDOW IS THE VALUATION DATE TO MONTH END - THREE DAYS.
  //
  // computeProjectWip takes costs where date > valStr AND date <= monthEndStr.
  // With a valuation day of 28, that is the 29th, 30th and 31st. The previous
  // version dated these costs "within the last 26 days", which put them in the
  // current month and outside every closed month's window - so the WIP page
  // showed nothing for August while the data looked present.
  //
  // Now: a few lines in the 29th-31st of each of the last six months, so every
  // month the user can select has WIP in it.
  if (!complete) {
    for (let m = 0; m < 6; m++) {
      const base = monthsBack(m)
      const dim = new Date(base.getFullYear(), base.getMonth() + 1, 0).getDate()
      if (dim <= 28) continue
      const postCount = intBetween(r, 1, 3)
      for (let c = 0; c < postCount; c++) {
        const acct = pick(r, COST_ACCOUNTS)
        const d = new Date(base.getFullYear(), base.getMonth(), intBetween(r, 29, dim), 12)
        const share = Math.round(between(r, 400, 9000))
        costLines.push({
          date: iso(d),
          supplier: acct.type === 'Labour' ? pick(r, SUBBIES) : pick(r, SUPPLIERS),
          reference: 'INV' + intBetween(r, 10000, 99999),
  ...
  const invoiceLines = applications.map((a, idx) => {
    ...
    // A QUARTER OVERDUE, NOT MOST.
    // 0.46 on live projects put nearly every invoice in the debtor book.
    const unpaid = complete ? r() < 0.06 : r() < 0.22
