...
export async function refreshXeroToken(refreshToken) {
  ...
  // THE STATUS AND THE BODY, NOT JUST "IT FAILED".
  //
  // This threw a bare string, so every caller that reported the message told
  // you nothing. Xero's body is the diagnosis: `invalid_grant` means the
  // refresh token has been consumed or revoked (Xero ROTATES it on every
  // use, so two concurrent refreshes leave one caller holding a dead one),
  // `invalid_client` means the credentials are wrong, and a 429 means rate
  // limiting. Those are three completely different jobs.
  if (!res.ok) {
    let body = ''
    try { body = (await res.text() || '').slice(0, 300) } catch {}
    throw new Error(`Token refresh failed: ${res.status}${body ? ` ${body}` : ''}`)
  }
  return res.json()
}
...
export async function fetchVatPosition(accessToken, tenantId, fromDateStr, toDateStr) {
  ...
  async function pullTyped(endpoint, { type, statuses, field, sign, key }) {
    ...
    let page = 1, count = 0
    while (page <= 300) {
      // FILTER BY DATE AT XERO, NOT HERE.
      //
      // This asked for every document of the type - ever - and threw away
      // everything outside the period in JavaScript. On purchase bills, the
      // biggest table there is, that meant 136 pages before Xero returned
      // 504 Gateway Time-out and the loop gave up with ZERO bills loaded.
      //
      // Input VAT is the whole refund side, so the estimate came out as sales
      // VAT with no purchases: every month read as payable when every month
      // was a refund of 17k to 50k.
      //
      // Asking Xero for the period fetches a fraction of the data and is the
      // actual fix. The two below are belt and braces for when it still fails.
      const clauses = []
      if (type) clauses.push(`Type=="${type}"`)
      if (fromDateStr) clauses.push(`Date>=DateTime(${fromDateStr.replace(/-/g, ',')})`)
      if (toDateStr) clauses.push(`Date<=DateTime(${toDateStr.replace(/-/g, ',')})`)
      ...
      meta.pages++
      // A TIMEOUT IS WORTH RETRYING. 504 and 503 are Xero being busy, not a
      // refusal - backing off and asking again usually works, and giving up
      // silently is what produced a wrong VAT figure.
      if ((res.status === 504 || res.status === 503 || res.status === 429) && attempt < 3) {
        attempt++
