...
// TITLE CASING FOR A GLOSSARY PHRASE.
//
// term('application') returns lower case: 'application for payment',
// 'payment claim', 'progress claim'. A blunt capitalise-every-word gives
// "Application For Payment", so the joining words are left alone.
const SMALL_WORDS = new Set(['for', 'of', 'and', 'the', 'a', 'an'])
export function claimTitle(t) {
  return String(t || '').split(' ')
    .map((w, i) => (i > 0 && SMALL_WORDS.has(w) ? w : w.charAt(0).toUpperCase() + w.slice(1)))
    .join(' ')
}

// The plural, for a nav item and a list heading. English does not pluralise
// these the same way: "Applications for Payment" moves the s to the FIRST
// word, "Payment Claims" to the last. The joining word is the tell.
export function claimTitlePlural(t) {
  const words = claimTitle(t).split(' ')
  const joinAt = words.findIndex((w) => SMALL_WORDS.has(w.toLowerCase()))
  const i = joinAt > 0 ? joinAt - 1 : words.length - 1
  words[i] = words[i] + 's'
  return words.join(' ')
}

export function describeApplication(app, opts = {}) {
  // `term` is the tenant's glossary function, passed in by the caller. See
  // the note on `claim` below for why it is not read here.
  const { prevReleases = null, term: termFn = null } = opts
  ...
  // WHAT THIS DOCUMENT IS CALLED, PER CUSTOMER.
  //
  // In New Zealand this is a PAYMENT CLAIM - the Construction Contracts Act
  // uses that name and attaches response deadlines to it. In Australia it is
  // a progress claim, in the US a pay application.
  //
  // The word is threaded IN rather than read here: this file is imported by
  // client pages, so it cannot import lib/locale.js (AsyncLocalStorage) any
  // more than TenantProvider can. The default is the UK phrasing, so a caller
  // that does not pass one behaves exactly as before.
  const claim = claimTitle(termFn ? termFn('application') : 'application for payment')
  const FINAL_TITLE = `Proposed Final Account and Interim ${claim}`
  const title = isFinal ? FINAL_TITLE : (releases.length ? releases.join(' & ') : claim)
