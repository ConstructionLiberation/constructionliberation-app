import { baseUrl, fromEmail } from './tenantSettings'
...
// TAKES THE RESOLVED ADDRESS, NOT THE FUNCTION THAT RESOLVES IT.
//
// The parameter was called baseUrl, which SHADOWED the imported baseUrl() at the top of
// this file, while the template below referenced portalUrl - a name that exists in two
// other functions here but not in this one. So every call threw:
//
//     ReferenceError: portalUrl is not defined
//
// A half-finished rename. It compiles, the import graph is clean, and npm run build
// never runs it - exactly the shape the comment further down this same file warns
// about. It only fails when the function actually runs, which is once a day.
//
// Consequence: crm-daily-activities has failed EVERY DAY since this was written. The
// daily activity emails have never been sent to anybody.
function buildEmail(group, today, portalUrl) {
  ...
  const html = `
    <div style="font-family:system-ui,Arial,sans-serif;max-width:640px;color:#1a1a19">
      ...
      <p style="margin-top:22px"><a href="${portalUrl}/crm" style="background:#2a7de1;color:#fff;text-decoration:none;padding:9px 16px;border-radius:8px;font-size:13px;font-weight:600">Open the CRM</a></p>
...
export async function sendNoActivityEmail({ dryRun, stageLabels = {} } = {}) {
  ...
  // ONE EMAIL PER ADDRESS, NOT ONE PER USER.
  //
  // This loops below and sends individually, so two portal users sharing an
  // address got two identical emails. On the demo tenant, where three seeded
  // estimators and the admin all carry the same address, that was FOUR copies
  // of the same message every day.
  //
  // It is not only a demo problem: a shared accounts@ or estimating@ mailbox
  // on two user records does the same thing on a real customer.
  //
  // lib/ramsNotify.js already dedupes its recipient list before sending. Same
  // rule, written in one place and not the other nine - see the note at the
...
export async function sendDailyActivityEmails({ dryRun } = {}) {
  ...
  for (const g of groups) {
    // portalUrl - the resolved string from a few lines above - not baseUrl, which is the
    // function. Passing the function is what left the template with nothing to read.
    const { subject, html } = buildEmail(g, today, portalUrl)
    try {
