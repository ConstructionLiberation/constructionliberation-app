...
export async function sendMentionEmails({ dealId, dealTitle, body, author, kind = 'note' }) {
  ...
  return notifyMentions({
    ...
    link: `${portalUrl}/crm?deal=${encodeURIComponent(dealId)}`,
...
// THE MISSING EXPORT.
//
// pages/api/crm.js has imported sendTestMention from this file since the
// mention work landed. This file exported six things and never that one. SWC
// compiles a missing named import to undefined rather than failing, so the
// build passed every time and the "send test mention" button on
// pages/crm-scores.js threw a 500 on every click.
//
// It sends ONE real email down the identical path a real mention uses - same
// resolver, same template, same sender - because a test that takes a different
// route proves nothing about the route that matters. The recipient is supplied
// rather than resolved, so it works before anyone is mentionable.
export async function sendTestMention(to) {
  const address = String(to || '').trim()
  if (!address) return { ok: false, sent: 0, error: 'No address given.' }

  const who = { id: 'test', name: 'Test Recipient', email: address }
  const out = await notifyMentions({
    body: `@${who.name} - this is a test of the mention email path.`,
    author: 'Portal diagnostics',
    users: [who],
    what: 'a test',
    context: 'Mention email test',
    link: baseUrl('PORTAL_BASE_URL') + '/crm',
    cta: 'Open the CRM',
    notifyAuthor: true,
  })
  return { ...out, testSentTo: address }
}
...
export async function diagnoseMentions(body) {
  ...
  return {
    ...
    resendConfigured: !!process.env.RESEND_API_KEY,
    from: fromEmail('forms'),
    baseUrl: baseUrl('PORTAL_BASE_URL'),
  }
}
