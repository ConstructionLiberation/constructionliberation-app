...
const shell = (inner) => `<div style="font-family:system-ui,Arial,sans-serif;max-width:640px;margin:0 auto;color:#1a1a19">${inner}<p style="font-size:12px;color:#999;margin-top:24px">${companyName()}</p></div>`
...
export function rockDrawingDigestHtml({ name, projectName, projectNo, comments, docs }) {
  ...
  return shell(`
    <h2>Hi ${esc(name) || 'there'},</h2>
    <p>${digestLine(comments, docs, 'Drawings', projectName || projectNo)}</p>
    ${btn(link, 'Open Drawings')}
