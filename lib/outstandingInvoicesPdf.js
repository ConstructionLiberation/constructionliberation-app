...
import { companyName } from './tenantSettings'
...
// The PDF ATTACHED to the weekly chase report. pkg957 fixed the email body
// and missed this, so the mail said $ and the attachment said £ on the same
// send. Server side, inside a tenant context, so it reads locale.js
// directly. The NaN case keeps returning a formatted zero rather than an
// empty cell, which is what the old one did.
const money = (n) => { const v = parseFloat(n); return tenantMoney(isNaN(v) ? 0 : v) }
const fmtDate = (s) => {
  if (!s) return '—'
  let d = null
...
export async function buildOutstandingInvoicesPDF({ invoices, includeComments = true, logoUrl, title = 'Outstanding Invoices' }) {
  ...
  pages.forEach((p, i) => {
    p.drawText(`${companyName()} — Outstanding Invoices · Page ${i + 1} of ${pages.length} · ${fmtDT(Date.now())}`, { x: M, y: 22, size: 7.5, font, color: GREY })
  })
