...
export async function cronHealth() {
  ...
  for (const name of names) {
    ...
    // "NOT CONFIGURED" IS NOT A FAILURE, AND IT IS TESTED BEFORE OVERDUE.
    //
    // A job whose integration does not exist returns { ok: false, error: 'Not
    // configured...' } and never records a fresh heartbeat - so it is ALWAYS
    // overdue as well. This branch used to sit after the OVERDUE test, which
    // meant OVERDUE always won and 'not configured' was unreachable for exactly
    // the jobs it was written for. The state existed, was correct, and never
    // ran. (crm-call-volume alerted as OVERDUE every day on that account.)
    //
    // Treating it as a failure sends the same email every morning for as long
    // as the integration stays on the backlog - and an alert that always says
    // the same thing is an alert people stop opening, which costs you the one
