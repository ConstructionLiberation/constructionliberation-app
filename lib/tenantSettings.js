...
export function fromEmail(kind = 'notify') {
  ...
  if (t) {
    ...
    const override = senders[kind] || (kind === 'notifyOnly' ? senders.notify : null)
...
// WHERE A NOTIFICATION GOES WHEN NOBODY ELSE CAN BE RESOLVED.
//
// This read fromTenant('replyTo') - a copy of the function directly above it,
// which is where it came from. Under one customer the two are the same address
// so nothing looked wrong. Under two, a last-resort alert went to the customer's
// PUBLIC reply-to rather than to whoever watches alerts, which is a different
// mailbox on purpose.
//
// A customer's own alertEmail wins. Falling back to their replyTo is still the
// right behaviour - an alert nobody receives is worse than one in the wrong
// inbox - but it is now deliberate rather than a typo, and it stays inside that
// customer. Never Rock's address.
export function alertEmail() {
  const t = currentTenant()
  if (t) return t.alertEmail || fromTenant('replyTo')
