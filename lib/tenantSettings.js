...
const ENV_CHAIN = {
  ...
  // NOTIFY with no FORMS fallback - pages/api/design-email-test.js. Not the same
  // chain as 'notify', so not the same kind.
  notifyOnly: ['NOTIFY_FROM_EMAIL'],
}
const LEGACY_FROM = {
  notify: 'Rock Roofing <onboarding@resend.dev>',
  notifyOnly: 'Rock Roofing <onboarding@resend.dev>',
...
export function fromEmail(kind = 'notify') {
  const t = currentTenant()
  if (t) {
    // A CUSTOMER MAY SEND AS MORE THAN ONE IDENTITY.
    //
    // Rock does. Notifications come from Rock Roofing <notifications@
    // send.rockroofing.co.uk>; applications come from Rock Roofing Accounts
    // <accountsreceivable@rockroofing.co.uk>. Both are real, both are what
    // their customers already recognise, and a single sender per customer would
    // have started sending applications from the notifications address the
    // moment tenancy was switched on.
    //
    // So: an optional override per kind, falling back to the customer's default
    // sender. Most customers will set one and never think about it.
    const senders = t.senders || {}
    // 'commercial' is the OLD NAME for 'accounts' and is still honoured,
    // because tenant records in the wild already set it - Rock's does. Code
    // should ask for 'accounts'; a record may answer to either. Remove the
    // alias once no record uses the old key.
    const override = senders[kind]
      || (kind === 'accounts' ? senders.commercial : null)
      || (kind === 'commercial' ? senders.accounts : null)
      || (kind === 'notifyOnly' ? senders.notify : null)
...
export function baseUrl(envName = 'PORTAL_BASE_URL') {
  const t = currentTenant()
  if (t) {
    const hosts = t.hosts || []
  ...
  // Single tenant: EXACTLY the chain the call site had. Three different ones
  // exist - PORTAL_BASE_URL, PORTAL_URL, APP_URL - all with the same default.
  // Trying all three would change the answer for a site that names only one.
  return process.env[envName] || 'https://app.rockroofing.co.uk'
}

export function ramsApproveOrigin() {
  const t = currentTenant()
  if (t) return baseUrl()
  return process.env.RAMS_APPROVE_ORIGIN || 'https://app.rockroofing.co.uk'
}
...
// --- Microsoft mailboxes ---------------------------------------------------
// Each customer has their own Microsoft tenant, so these are per-customer too.
// Same rule as the rest: their value, or a loud failure. Never Rock's.
// IS MICROSOFT CONFIGURED FOR THIS CUSTOMER.
//
// msSyncGroup() and msBccMailbox() THROW when a customer has not set them, which
// is right - a silent fallback to Rock's group is exactly the accident this file
// exists to prevent. But a cron that loops every tenant needs to ask the question
// without being thrown at, or a customer who does not use Outlook sync produces
// an error email every fifteen minutes for ever.
//
// Ask this first, and skip cleanly. Single tenant (no tenant in scope) keeps the
// old behaviour: configured if the environment says so.
export function msConfigured() {
  const t = currentTenant()
  if (t) return !!(t.msSyncGroup && t.msBccMailbox)
  return !!(process.env.MS_SYNC_GROUP || process.env.MS_BCC_MAILBOX)
}

export function msSyncGroup() {
  const t = currentTenant()
  if (t) return fromTenant('msSyncGroup')
  return process.env.MS_SYNC_GROUP || 'crm-sync@rockroofing.co.uk'
}

export function msBccMailbox() {
  const t = currentTenant()
  if (t) return String(fromTenant('msBccMailbox')).toLowerCase()
  return (process.env.MS_BCC_MAILBOX || 'crm@rockroofing.co.uk').toLowerCase()
}

// Used to tell internal email from external. Getting this wrong for a customer
// would count their own staff as clients, so it fails rather than guesses.
export function msInternalDomain() {
  const t = currentTenant()
  if (t) return String(fromTenant('msInternalDomain')).toLowerCase()
  return (process.env.MS_INTERNAL_DOMAIN || 'rockroofing.co.uk').toLowerCase()
}
