...
import { tenancyEnabled } from './tenants'
...
// THE ONE PLACE A DATABASE IS CHOSEN.
//
// This is the function EVERY helper below calls - getProject, saveProject, get,
// set, getPortalUsers, all 20 of them, used by 164 files. It used to return the
// global client from the environment variables and nothing else.
//
// Which meant that with tenancy switched on, only the files converted in
// pkg843-852 to call getClient() were actually reading the customer's database
// through the registry. Everything using a named helper - which is most of the
// app - was still reading whatever the environment variables pointed at.
//
// Nothing looked wrong, because for Rock those are the same database. It would
// have looked wrong the moment a second customer existed, and by then the
// symptom would have been one company seeing another's projects.
//
// Resolving the tenant HERE rather than in getClient() is the fix, and it is
// the property the 49-file conversion was supposed to buy in the first place:
// one function knows which database, and everything else follows without
// knowing anything.
async function getRedis() {
  // A customer in scope means their database, and only after it has proved it
  // is theirs.
  ...
  // NO CUSTOMER IN SCOPE, WITH TENANCY ON: REFUSE.
  //
  // This is the guardrail the whole exercise has been building towards.
  //
  // Until now, a route that was never wrapped in withTenant fell through to the
  // global client and read ROCK'S database. It worked. It looked completely
  // normal. Serving it to a different customer would have produced plausible
  // figures belonging to the wrong company, and nothing anywhere would have
  // said so.
  //
  // Now it throws. A missed route becomes a broken page - loud, immediate, and
  // fixable in one line - instead of a silent leak nobody finds.
  //
  // This is deliberately louder than falling back to the in-memory store, which
  // would be the other option and would look like "no data" rather than "this
  // code is wrong".
  if (tenancyEnabled()) {
    throw new Error(
      'No customer in scope. This request reached the database without resolving a customer, '
      + 'which usually means the API route is not wrapped in withTenant (or a cron is not using '
      + 'forEachTenant). Refusing rather than reading the default database.'
    )
  }

  // Single tenant: the global client, exactly as the app has always run.
  try {
...
// A CLIENT FOR AN EXPLICITLY NAMED DATABASE.
//
// Everything else in the app must use getClient(), which resolves the customer
// from the request. This exists for exactly one job: stamping a customer's
// identity record into their database BEFORE that customer can be resolved -
// the chicken-and-egg at the start of provisioning.
//
// It deliberately does NOT run the identity self-check, because the whole point
// is that the identity is not there yet. That makes it the one place in the app
// that can write to a database without proving whose it is, which is why it is
// used by a single admin-only endpoint and nowhere else.
export async function clientForCredentials({ url, token }) {
  if (!url || !token) throw new Error('A database url and token are both required.')
  const { Redis } = await import('@upstash/redis')
...
export async function getClient() {
  // getRedis() now does the resolving, so this is simply the client-shaped
  // wrapper around it. Two functions each deciding which database is two rules
  // that can disagree; there is one.
  const redis = await getRedis()
  if (redis) {
    return {
      get: (k) => redis.get(k),
