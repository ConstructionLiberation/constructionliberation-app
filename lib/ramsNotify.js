import { fromEmail, alertEmail, companyName } from './tenantSettings'
import {
  get, set,
...
function shell(bodyHtml) {
  return `<div style="font-family:system-ui,-apple-system,sans-serif;max-width:560px;margin:0 auto;color:#1a1a19">${bodyHtml}<p style="font-size:12px;color:#999;margin-top:24px">${companyName()} Site App</p></div>`
}
  ...
  try {
    ...
    await sendEmail({
      ...
      html: shell(`
        ...
        <p style="font-size:15px">As ${companyName()} Contracts Manager for <strong>${label}</strong>, RAMS ${fileName ? `(<strong>${fileName}</strong>) ` : ''}have been uploaded and need your approval to start the sign-off process.</p>
  ...
  try {
    ...
    const sent = await sendEmail({
      ...
      html: shell(`
        ...
        <p style="font-size:15px">The Contracts Manager has approved the RAMS ${fileName ? `(<strong>${fileName}</strong>) ` : ''}for <strong>${label}</strong>. As ${companyName()} Director, please review and sign onto them in the Site App.</p>
        <p style="text-align:center;margin:24px 0"><a href="${SITEAPP_URL}" style="background:#ca8a04;color:#fff;text-decoration:none;padding:12px 24px;border-radius:10px;font-weight:600;display:inline-block">Approve in the Site App</a></p>`),
    })
...
export async function notifySiteManagerRejection({ projectNo, fileName, byName, notes, cmName, directorName }) {
  try {
    ...
    // THE CUSTOMER'S ALERT ADDRESS, NOT THE DEPLOYMENT'S.
    //
    // This read process.env.ALERT_EMAIL, which has ONE value for the whole
    // deployment. With a second customer, their RAMS rejection with no CM and no
    // Director resolved would have been posted to Rock Roofing's inbox - another
    // company's operational mail, arriving silently, with nothing saying so.
    const fallback = alertEmail()
    if (!uniq.length && fallback) uniq = [fallback]
    if (!uniq.length) { console.error('sm-reject: no recipients resolved (CM/Director/alert address all empty)'); return }
...
export async function notifyAllocation({ projectNo, opId }) {
  try {
    ...
    if (isProjectCM) {
      await sendEmail({
        to: email,
        subject: `${companyName()} project allocated — ${label}`,
        html: shell(`
          <h2 style="color:#1a1a19">${companyName()} project allocated</h2>
          <p style="font-size:15px">Hi ${op.firstName || 'there'},</p>
          <p style="font-size:15px">You've been allocated as <strong>Contracts Manager</strong> on the ${companyName()} project <strong>${label}</strong>.</p>
