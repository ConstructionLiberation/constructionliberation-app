...
export async function buildPreStartPDF({ project, data, logoUrl, proof, sections }) {
  ...
  const drawText = (text, { f = font, size = 10, color = INK, indent = 0, gap = 4 } = {}) => {
    ...
    // SANITISE HERE, not at every call site. This helper is the only route to
    // page.drawText in this file, so one call covers all 25 of them - and covers
    // whatever is added next without anyone remembering.
    //
    // Without it, a curly quote or an em-dash pasted into a Pre-Start field made
    // pdf-lib THROW, which failed the whole PDF and therefore the send. The text
    // most likely to contain one is exactly the text somebody has pasted out of
    // an email into the minutes.
    for (const line of wrap(pdfText(text), f, size, maxW)) {
      ensure(size + gap)
