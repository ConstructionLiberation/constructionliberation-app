...
import { term } from './locale'
import { claimTitle, describeApplication, computeApplicationSummary, worksValueToDate, isMeasurableWorks, variationValue, variationValueToDate, materialLineTotal, materialValueToDate, isInstructed } from './applications'
...
// One rule, in lib/pdfText.js. This version kept the pound sign - correctly -
// but replaced every accented character with a space, so a customer or contact
// name with one came out mangled on a document that goes to the customer.
const san = pdfText
...
export async function buildApplicationPDF({ app, appNumber, project = {}, prevGross = 0, prevReleases = null, trackerVariations = [], logoUrl }) {
  ...
  // What the customer's jurisdiction calls this document. Title-cased once
  // and reused for every label below.
  ...
  metaRow(`${claim} no.`, String(appNo))
  metaRow(`${claim} date`, fmtDMY(appDate))
  ...
  heading('Summary')
  {
    ...
    drawHeader(['', 'Contract Sum', `${claim} Total`, 'Proj. Final Account'])
    ...
    // GROSS. Before MCD and before retention - both of which are applied in the
    // certificate below, not here. Called "Application Total" it reads as the figure
    // being claimed, sits directly under two retention lines it does not include, and
    // invites exactly one conclusion: that the total is wrong. It is not; the name was.
    row(`Gross ${claim} Total`, '', money(sum.applicationTotal), '', true)
