...
function renderHtml(body, replyTo) {
  ...
  const foot = replyTo
    ? `Sent automatically from the ${companyName()} portal. Replies go to ${esc(replyTo)}.`
    : `Sent automatically from the ${companyName()} portal. Replies to this address are not monitored.`
...
async function sendEmail(to, subject, body, replyTo) {
  ...
  // Customer's sender. See the note in lib/designEmail.js - the environment
  // used to win, so fromEmail() never ran.
  ...
  const r = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: FROM, to, subject: subject || `${companyName()} notification`,
