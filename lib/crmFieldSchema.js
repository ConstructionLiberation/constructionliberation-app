...
export const DEFAULT_FIELD_SCHEMA = [
  ...
  // CUSTOMER TYPE. Read in about ten places across the Sales Dashboard and
  // both scorecards - and until now written NOWHERE. crmDashboardAdapter had
  // `customerType: null` hardcoded, so "Value priced - existing customers"
  // returned 0 on every tenant including Rock, against a 300k target, and the
  // dashboard filed every deal under Prospect because that is the fallback.
  //
  // A concept with ten consumers and no producer. This is the producer.
  { key: 'customer_type', label: 'Customer Type', type: 'select', group: 'details',
    options: ['Existing Customer', 'New Customer', 'Prospect'] },
