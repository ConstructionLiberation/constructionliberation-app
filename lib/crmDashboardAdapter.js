...
export function crmDealToFlat(d, milestone) {
  ...
  return {
    ...
    // Was hardcoded null. The note at the top of this file says customerType
    // is not in the Pipedrive export and would be "captured going forward" -
    // it is now captured, as customer_type on the deal, so this reads it.
    // A deal nobody has set falls back to Prospect, which is what the
    // dashboard was already showing everything as.
    customerType: f.customer_type || 'Prospect',
    wonCount: 0,
  }
}
