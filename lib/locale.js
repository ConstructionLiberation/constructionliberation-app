...
//
// lib/businessDate.js has taken a `tz` argument since it was written, with a
// comment saying the tenant record would supply it one day. It never did:
// DEFAULT_TZ is Europe/London and not one of the twenty-two call sites passes
// anything. Same shape as logoUrl() - correct, tenant-aware, no consumers.
//
// A wrong timezone is not cosmetic for a customer twelve hours away. NZ is
// UTC+12, so for most of their working day "today" computed in London is
// YESTERDAY, and a valuation date, a week anchor and an overdue flag all move
// with it.
//
// An explicit timezone on the record wins, exactly as currency does, so a
// customer can run an NZ locale on UK time if that is genuinely their
// business. An INVALID one does not win - it falls through to the locale
// default rather than throwing inside Intl on every date on the page.
const TZ_BY_LOCALE = {
  UK: 'Europe/London',
  US: 'America/New_York',
  AU: 'Australia/Sydney',
...
export function businessTimezone() {
  const t = currentTenant()
  const explicit = String((t && t.timezone) || '').trim()
  if (explicit && tzIsReal(explicit)) return explicit
  return TZ_BY_LOCALE[localeKey()] || TZ_BY_LOCALE.UK
}
