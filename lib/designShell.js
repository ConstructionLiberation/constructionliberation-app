...
export const DESIGN_PAGES = [
  ...
  // HIDDEN, NOT DELETED. Unfinished rather than unwanted - the page still
  // exists, so remove `hidden: true` to bring it back rather than rebuilding it.
  // Same convention as components/OperationsNav.js.
  { key: 'tech-sub-builder', label: 'Tech Sub Builder', slug: 'tech-sub-builder', internalOnly: true, hidden: true },
  { key: 'tech-sub', label: 'Tech Sub', slug: 'tech-sub' },
  // LABEL only. The key and the slug are the route and the stored category -
  // /design/<project>/rock-drawings, design-rock-drawings, the
  // design-rockdrawing-daily cron - and renaming those would break links
  // people have bookmarked and records already saved.
  { key: 'rock-drawings', label: 'Drawings', slug: 'rock-drawings' },
  { key: 'calculations', label: 'Calculations', slug: 'calculations' },
  // Label only, same reasoning as Drawings above.
  { key: 'leak-test-certs', label: 'Testing', slug: 'leak-test-certs' },
...
export function DesignNav({ active, projectNo, projectName, isInternal }) {
  // The design portal is the ONE area a customer's own client logs in to, so
  // it is the last place another company's logo should appear.
  const { companyName: brand, logoUrl: logo } = useFormat()
  const pages = DESIGN_PAGES.filter(p => !p.hidden && (!p.internalOnly || isInternal))
  return (
    <div>
      <div style={{ background: '#1a1a19', padding: '0 20px', display: 'flex', alignItems: 'center', height: 52, overflowX: 'auto' }}>
        {logo ? <img src={logo} alt={brand} style={{ height: 30, width: 30, borderRadius: 4, marginRight: 8, flexShrink: 0 }} /> : null}
