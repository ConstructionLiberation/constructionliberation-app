...
export async function reportError({ where, error, req, tenantId }) {
  try {
    const key = process.env.RESEND_API_KEY
    const to = process.env.ALERT_EMAIL || process.env.FORMS_REPLY_TO
    ...
    if (throttled(route + '|' + message.slice(0, 120))) return
