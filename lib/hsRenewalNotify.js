import { fromEmail, replyTo, companyName } from './tenantSettings'
...
async function buildPeople() {
  ...
  for (const u of (portal || [])) {
    const name = [u.firstName, u.lastName].filter(Boolean).join(' ') || u.name || ''
    if (!name) continue
    map[`pu:${u.id}`] = { name, email: u.email || '', company: `${companyName()} (office)` }
  }
  return map
}
...
async function sendMail({ to, subject, html }) {
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from, to: Array.isArray(to) ? to : [to], reply_to: replyToAddr, subject, html }),
    })
    return { sent: r.ok }
    ...
    const html = `
      <div style="font-family:system-ui,Arial,sans-serif;max-width:560px;color:#1a1a19">
        ...
        <div style="background:${r.overdue ? '#fef2f2' : '#fff7ed'};border:1px solid ${r.overdue ? '#fecaca' : '#fed7aa'};border-radius:10px;padding:14px;margin:16px 0;font-size:15px">
          Please send your renewed ${esc(what)} to ${companyName()} to keep your training up to
          date and to maintain your ability to work for ${companyName()}.
        </div>
        <p style="font-size:13px;color:#666">Reply to this email with a photo or scan of the renewed card or certificate.</p>
        <p style="font-size:12px;color:#999;margin-top:20px">${companyName()}</p>
      </div>`
