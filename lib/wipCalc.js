...
export function computeProjectWip({ costLines = [], invoiceLines = [], valStr, monthEndStr, adjustments = [], marginOverride = null }) {
  ...
  // NO FLOOR AT ZERO.
  //
  // This was Math.max(0, wipValue), so a project whose adjustments exceed its
  // post-valuation costs reported 0.00 instead of the negative figure it is.
  //
  // WIP CAN BE NEGATIVE, and when it is, that is the number that matters: it is
  // a write-down - cost you have decided you will not recover, or work
  // over-valued in an earlier month and being pulled back. Flooring it to zero
  // hides exactly the adjustment somebody has gone to the trouble of entering.
  //
  // J244 Waungron: post-valuation costs 1,234.48, adjustments -4,500.00. The
  // real figure is about -4,060, and the page showed 0.00 with a profit of
  // 3,265.52 that existed only because the floor had moved the cost.
