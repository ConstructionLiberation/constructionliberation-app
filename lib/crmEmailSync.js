import { msSyncGroup, msBccMailbox, msConfigured } from './tenantSettings'
...
// THESE WERE EVALUATED AT MODULE SCOPE AND IT PUT ONE CUSTOMER'S MAIL IN ANOTHER'S
// DATABASE.
//
// `export const SYNC_GROUP = msSyncGroup()` runs ONCE, when the module is first
// loaded by a serverless instance - which happens with NO tenant in scope. So it
// took the environment fallback, 'crm-sync@rockroofing.co.uk', and then stayed
// frozen at that value for every tenant that instance went on to handle.
//
// The consequence, seen on 19 September 2026: the crm-email-sync cron looped to
// the zztest tenant, read ROCK'S mailboxes, and wrote Rock's messages into
// zztest's database as crm:emails:unallocated and crm:emails:sync-state. The
// Redis isolation held perfectly throughout. The data crossed on a module-scope
// constant and a global Graph credential, which is a boundary nothing else
// watches.
//
// Worse: zztest's registry record has no msSyncGroup, so fromTenant() would have
// THROWN and refused. The guard existed and was correct. Evaluating at module
// scope meant it was never called while zztest was in scope, so the guard never
// ran. The warning about this pattern is written at the top of tenantSettings.js
// and was two imports away.
//
// RULE: never call a tenant-aware function at module scope. It must be called
// inside the request, every time, or it is not tenant-aware at all.
//
// Kept as functions with the old names so every call site reads the same.
export const syncGroup = () => msSyncGroup()
export const bccMailbox = () => msBccMailbox()

// Mailboxes that only ever match on an exact project title in the subject.
// Still an environment variable, so still ONE value for the whole deployment.
// That is acceptable only while it is empty, which it currently is. It belongs
// on the tenant record with the other Microsoft settings.
...
export async function runEmailSync({ backfillMonths = 0, dryRun = false, max = 2000, detail = false, since: sinceOverride = '', until = '', mailbox: onlyMailbox = '', queue = true } = {}) {
  ...
  // This customer does not use Outlook sync. Skip, do not throw - a cron that
  // loops every tenant must not error its way through the ones it does not apply
  // to. Without this the sync would read the deployment's mailboxes instead,
  // which is the fault this package exists to fix.
  if (!msConfigured()) return { ok: false, skipped: true, error: 'Microsoft is not configured for this customer' }
  ...
  let mailboxes = await listSyncMailboxes(syncGroup())
  ...
  for (const mb of mailboxes) {
    ...
    const isBcc = mb.email === bccMailbox()
