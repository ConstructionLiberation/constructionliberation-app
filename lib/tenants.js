...
// VERCEL NAMES THINGS ITSELF.
//
// Creating the store through Vercel's Upstash integration with the prefix
// CONTROL_REDIS produced CONTROL_REDIS_KV_REST_API_URL, not CONTROL_REDIS_URL.
// So both spellings are accepted, exactly as lib/db.js already accepts three
// different spellings for Rock's database.
//
// The alternative was copying the values into a second pair of variables by
// hand, which works until Vercel rotates the credentials and the copy silently
// goes stale. Reading the name Vercel actually set has no such failure.
function controlUrl() {
  return process.env.CONTROL_REDIS_URL
    || process.env.CONTROL_REDIS_KV_REST_API_URL
    || null
}

function controlToken() {
  return process.env.CONTROL_REDIS_TOKEN
    || process.env.CONTROL_REDIS_KV_REST_API_TOKEN
    || null
}
